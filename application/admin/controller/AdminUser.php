...
class AdminUser extends Common
{
    ...
    public function userList()
    {
        if ($this->request->isAjax()) {
            ...
            $list = AuthAdminModel::withSearch(['name'], ['name' => $data['key']])
                ->hidden(['password'])
    ...
    {
        if (!$this->request->isAjax()) {
            $info = AuthAdminModel::get($this->uid)->hidden(['password']);
            ...
            $res = AuthAdminModel::update($data, ['uid' => $this->uid]);
            if ($res) {
                $code = 1;
                $msg = '保存成功';
            }else{
    ...
    public function edit()
    {
        if ($this->request->isPost()) {
            ...
            if ($data['uid']) {
                //编辑
                $res = AuthAdminService::edit($data);
                return $res;
            } else {
                //添加
                $data = AuthAdminService::add($data);
                return $data;
            }
        } else {
            $uid = $this->request->get('uid', 0, 'intval');
            if ($uid) {
                $list = AuthAdminModel::where('uid', '=', $uid)->hidden(['password'])->find();
    ...
    public function check()
    {
        ...
        $res = AuthAdminModel::where('user', '=', $username)->field('uid')->find();
        if ($res) {
    ...
    public function delete()
    {
        ...
        if ($uid) {
            if ($uid != 1) {
                $res = AuthAdminService::delete($uid);
                return $res;
            } else {
    ...
    public function editPassword()
    {
        if ($this->request->isPost()) {
            ...
            $res = AuthAdminService::editPassword($user['uid'], $data['oldpassword'], $data['password']);
            return $res;
        } else {
            return $this->fetch();
        }
